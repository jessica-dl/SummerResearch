...
def clustering_knn(data, k=K_CONST):
    """
...
def clustering_kmember(data, k=K_CONST):
...
def clustering_oka(data, k=K_CONST):
    """
...
def clustering_based_k_anon(att_trees, data, type_alg='knn', k=K_CONST, QI_num=-1):
    """
    ...
    if type_alg == 'knn':
        print "Clustering based on k-nearest neighbour algorithm"
        clusters = clustering_knn(data, k)
    elif type_alg == 'kmember':
        print "Clustering based on k-member algorithm"
        clusters = clustering_kmember(data, k)
    elif type_alg == 'oka':
        print "Clustering based on one time pass k-means algorithm"
        clusters = clustering_oka(data, k)
    else:
        print "Please choose a merge algorithm"
        print "knn | kmember"
        return (0, (0, 0))
